...
编辑器的 chrome 是**一个** Dockview：左栏场景图在上、基础组件（与可选历史）在下上下分栏，
中央画布，右栏属性，底部一条横跨全宽的边缘组承载资源 / 命令 / 日志（动画工作区还有时间线）。
文档（页面、组件、资源文件）不是 Dockview 面板而是编辑器会话，顶部一条横贯全宽的文档标签条
（`tablist`）在 Dockview 之外，右端是设置齿轮；左右两侧的组头上各有一个折叠按钮，收起后
沿编辑器边缘留一条 8px 把手。标尺不带底色与边框，只有刻度压在图面上；工具栏放不下时尾部的
按钮按顺序收进「更多」菜单。

**工作区**是这套 chrome 上面的一层：一份 Dockview 布局加一组画布会话开关（角度约束、增量角、
网格显示、十字光标臂长、变换指示器），顶部标签条右端的切换器随时切。它不是模式——不改任何
一次点击的含义、不改命令可用性、不写文档、不进撤销历史，正在取点的命令跨切换继续等它的点；
它也不绑定文档——每个文档记住上次在哪个工作区里编辑它，激活标签时自动切过去，切换器随时
能点。面板可拖，拖过的样子随工作区记住（`toJSON` 进偏好 `workspace.layouts`，对宿主不透明、
失配即回到定义自带的布局）；「另存为」把此刻的样子存成新工作区，「重置布局」拉回基线，
内建与宿主注入的不可删、不可重命名。宿主经 `workspaces` 注入自己的
（`[...COMPOSE_DEFAULT_WORKSPACES, mine]`，`id` 重名抛错），定义里只有布局（`preset` 或不透明
快照）、物料货架、会话开关与新建种子，没有任何能改命令集或按钮含义的字段。

内建有两个：**页面**与**绘图**。差别在货架与种子——绘图把工具的耗材铺开（符号库拉高、十字
光标贯穿图面、底部让位给图面），页面把基础组件与它们的面板铺开。会话开关只差十字光标臂长：
其余几个本来就按工作区各记一份、切回时恢复，默认值不必再分叉一次。
工作区各带一条**工具栏货架**：页面留选择、指示器、吸附、网格、容器、文字、矩形与箭头；绘图留
整套制图工具（八条绘图命令、正交与极轴），不留容器。收走的是**入口不是能力**——页面里 `CIRCLE`
照样能敲、能按 `C`；作为补偿，圆的物料瓦片会在页面工作区里出现（`paletteHidden` 的判据是「工具栏
是否已提供入口」，按当前货架求值）。管理菜单里的「自定义工具栏…」可以增删排序、插分隔与重置。

**两个货架用同一个对话框**：左边编排、右边来源，两边可以互相拖——从右边拖进来就上架，从编排区
拖回右边就移出，编排区内拖动即改顺序。键盘等价是「抓起—移动—放下」（空格抓起、方向键移动、
再空格放下、`Escape` 放弃、`Delete` 移出）。编排区**画的是它将来的样子**：工具栏排成一条横栏，
并在当前窗口会被收进「更多」的那一格之前画出切口；物料面板排成一列段卡，段的选项就在卡上。
来源列上每一条印着它的**第二条入口**（单键、命令词，或「命令面板」），因此「拿掉一格不会拿掉
那项能力」这句承诺是可核对的。物料那份的入口除了管理菜单，还挂在**组件面板标签的右键**上——
一个瓦片都没有的面板正是最需要配置的那一档。

工作区还带一份**物料货架**：物料面板按段渲染，一段是一个**来源**——Registry 的基础 Preset
（按单项勾）或资源里的一个文件夹（按子文件夹分组或平铺）。**选文件夹不选文件**：往
`Symbols / 变电站` 里再导十个符号，它们自动出现；想建新分类就去资源里建文件夹。文件夹不在了
那一段显示「找不到」并可去掉，不报错。绘图的货架是 `Symbols` 按子文件夹分组 + 项目组件 +
折叠的基础组件，带搜索；页面的是基础组件 + 项目组件平铺。两份货架都可以改：右键一块瓦片能把
基础组件从面板隐藏，右键文件夹来源那一段能「只看这一组」（文件夹里的瓦片**不能**单个隐藏，
否则「新导入的自动出现」就成了谎言）；「自定义物料面板…」里能改面板标题、排序来源、开关搜索框
与分组方式、勾选列出哪几个基础组件，以及添加一个新的文件夹来源。两个对话框的排序都是按钮驱动
而不是拖放——键盘重排本来就要写一遍，拖放是它之上的增强。
**新建种子**是工作区给新文档的默认值（网格步长、网格吸附与对齐吸附）：在绘图里新建页面得到
10×10 的网格（AutoCAD `SNAPUNIT` 的默认值）且**对齐吸附关**，在页面里得到 4×4（界面设计通行的
基础模数，8 仍然落在格点上）且对齐吸附开。
绘图关掉的只是 Figma 式参考线那一层——它与端点 / 端口的特征点捕捉是姐妹查询，同时生效会在同一次
取点里互相拉扯，而接线图上每一次落笔都在找端子；网格吸附照旧开着，徒手画的外框与分区框要靠它。
种子只在新建时落地——切换工作区不碰任何已有文档，也不产生事务，因此已经打开的页面切到绘图时
对齐吸附仍然开着，要关按工具栏那个吸附按钮（那本来就是一次文档编辑）。
...
ViewModel），而绑定是页面级平坦命名空间，按场景切分会让实体跨场景移动时静默失去绑定。时间线是
内建「动画」工作区里的一块面板；**动画编辑**是时间线 chrome 上的一个显式开关（按钮、
`document.toggleAnimationMode` 动作，或对时间线的第一次交互），开着时时间线接文档、属性面板
菱形打点、自动记录、画布运动路径编辑，时间线不再可见即退出；切换工作区永不打开它，
