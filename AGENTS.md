...
- **几何编辑是一个对象作用域的模式，复用运动路径那条既有通道。**双击一条曲线进入：夹点显形、
  ...
  **多段线的段中点夹点表示「在这里插入一个顶点」**——这兑现的正是当初把它留白时写下的理由
  （段中点是顶点增删的天然位置，用它表示平移会把那条路堵死）。多段线因此仍然没有平移夹点，
  那仍是声明过的例外；弧的平移仍是圆心（AutoCAD 没有弧圆心夹点，这是有意的偏离）。
  闭合多段线的段**含收尾那一段**：它在屏幕上与其他段没有区别，漏掉会让折线上恰好少一个可抓
  的位置而用户看不出为什么。id 用不同前缀（`v{顶点}` / `m{段}`）——求解要按 id 分派到两种
  完全不同的操作，而它手上只有 id。插入**每帧只产生一个**：拖动期每一帧都拿**文档**里的几何
  加同一个 id 重求一次，因此 `m2` 在任何一帧都表示「在第 2 段中间插一个」。
  **夹点形状按角色分，三个角色三种形状**：`keyframe` 菱形、`vertex` 方块、`insert` 沿段方向
  的条形。两个夹点长得一样而按下去做的事不同，是最难自己发现的一类缺陷。**菱形留给运动
  ...
  **「删除顶点」暂缺**：AutoCAD 把它挂在悬停菜单上，而我们没有菜单，随手发明一个键位是猜；
  加错了顶点用撤销退回去，这条路眼下不堵。
