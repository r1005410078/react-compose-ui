...
- **几何编辑是一个对象作用域的模式，复用运动路径那条既有通道。**双击一条曲线进入：夹点显形、
  ...
  **未拖动时线与框都画，拖住夹点之后框让位**：未拖动的会话正是「等着抓点什么」，框表达可抓
  的靶区；抓住之后那件事已经发生，框只会挡住落点。这与 AutoCAD 的 `Command:` / `Select
  objects:` / `Specify point:` 三态同构——「夹点显示着、没有命令在跑」在那边就是 `Command:`。
  **夹点有悬停态**（AutoCAD 的 `GRIPHOVER`）：可见菱形比命中区小，没有反馈时用户唯一能确认
  「按下去会抓到它」的办法是按下去试。它是纯 CSS（命中圆排在菱形之前，用 `:hover +` 读），
  **不为一个视觉反馈引入每帧 `pointermove` 都要写的 React 状态**。**不做「热夹点」**
  （`GRIPHOT`，拖动中变红）：它要表达「正在拖的是哪一个」，而夹点本来就跟着预览几何走在
  光标底下，那个信息已经在屏幕上了。
