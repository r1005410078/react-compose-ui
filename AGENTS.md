...
- **框选的判定默认是「方向决定」**：从左往右只选**完全框住**的（窗口），从右往左**碰到就选**
  （窗交）。默认落在某一种固定判定上时，方向决定那套代数等于不存在——不去菜单里手动选一次，
  拖拽方向就什么都不做，功能在而可达不到。这不是照搬 AutoCAD：两种判定各自都有真实用途
  （框住整根导线 / 抓一把穿过某片区域的线），而拖拽方向是它们之间最快的切换。菜单三项都留着，
  改的只是初值；「MUST NOT 改写用户选定的模式」约束的是**运行期的偷偷改写**，不是初值。
