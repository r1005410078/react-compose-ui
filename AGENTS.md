...
- **曲线是带盒的普通 Entity。** 线、弧与多段线由可选的 `Curve` Component 承载几何，
  ...
  **「矩形是闭合多段线」说的是 `Curve` 协议内部，不是 `RECTANGLE` 命令的产出。**那条判断管的
  是要不要另立一个 `rect` kind；而 `RECTANGLE` 命令产出的是**矩形物料**（带完整 `Appearance`
  的盒），因为判据是**用户画完之后想对它做什么**——画一个矩形外框，下一步九成是填色、调圆角、
  往里塞东西，这些 `Curve` 全都做不到。命令用 `boxes` 效果显式说出这个意图，**按 kind 反推是
  错的**：`PLINE` 画四点按 `C` 同样得到闭合四顶点折线，而那时用户要的确实是折线。真需要纯几何
  四边形轮廓时 `PLINE` 就是入口。从盒下钻到自由几何要一条显式的 `EXPLODE`（尚未做）而**不是
  双击**：双击今天是无副作用的会话入口，而转换会丢圆角与非 solid 背景；何况矩形的四个顶点恰好
  就是它包围盒的四个角，不转换的话拖顶点与拖盒角手柄逐像素相同。
...
- **几何编辑是一个对象作用域的模式，复用运动路径那条既有通道。**双击一条曲线进入：夹点显形、
  ...
  **「转为圆弧」是协议缺口不是工作量**——多段线顶点没有 bulge 字段，一条多段线里的弧段在当前
