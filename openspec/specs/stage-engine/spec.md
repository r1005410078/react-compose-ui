...
**两个操作数共用一段弧形边界时仍然求解退化，这是一处明写的边界而不是本条的目标**：那段弧在
两个对象里各存了一份、各自量化，而平面图只在**交点**处切边——两段近似共圆的弧求交给出的是没有
意义的点。把共圆的边归一成同一个圆是另一次变更。MUST NOT 靠调大节点合并容差绕过它：量过，
那样求解确实不再报错，产出的却是一个几像素大的退化形状——**把一个看得见的失败换成一个看不见的
错误，方向反了**。

#### Scenario: 由弧围成的填充块参与区域运算

- **WHEN** 一块由圆弧围成的填充（`path`）与一个压在它上面、不共用任何边界的矩形一起执行并集
- **THEN** 解算成功，产物的几何是两者的并集；不再以 `bezier` 拒绝

#### Scenario: 共用弧形边界的两个操作数给出可见的拒绝

- **WHEN** 两块相邻的填充共用同一段圆弧边界，对它们执行并集
- **THEN** 以「求解退化」拒绝，文档一个字节不变；MUST NOT 产出一个退化的形状
