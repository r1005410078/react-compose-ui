...
- **THEN** 提交效果里有一个 `boxes` 项，且没有任何 `curves` 项
...
**导线 MUST 只走横平竖直**：从第二个点起，`WIRE` 的取点提示 MUST 把角度约束钉死成正交，
与会话级的三态设置无关。这是**规范而不是偏好**——斜着走的导线在一次接线图上不是「用户的
选择」，是一张画错的图。第一个点没有上一点、量不出方向，MUST NOT 钉。

钉死的 MUST 只是落点解算里**角度约束那一档**，管线次序 MUST 原样成立
（`键入 > 捕捉 > 网格 > 角度约束`）。两条推论都是要的：**捕捉命中时短路**，最后一段够得着
端口而不会被正交挡在门外；**键入的坐标不被改写**，用户打出来的精确值仍然是精确值。

**MUST NOT 引入任何自动路由**：替用户补拐角要回答「拐点该拐在哪儿」，而真正的答案要考虑
障碍物，那是另一个子系统。用户点到哪儿就是哪儿——正交只把落点拽回轴向，不替他决定拐几次。
...
#### Scenario: 第二个点被钉死在正交上

- **WHEN** 会话级角度约束为关，`WIRE` 取过第一个点后把第二个点落在一个明显斜的位置
- **THEN** 落点被拽回与上一点同一水平线或同一垂直线上

#### Scenario: 端口捕捉压过正交

- **WHEN** 第二个点落在一个与上一点既不同行也不同列的端口上
- **THEN** 落点是那个端口，且该端绑到它——捕捉命中即短路，正交排在其后
