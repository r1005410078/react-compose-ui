...
- [ ] 1.1 `packages/core/src/page/page-types.ts`：页面后缀、页面媒体类型、清单文件名常量，
  `ComposeAppManifest`、`ComposePageReference`、`ComposePageDocumentLoader`、`ComposePageNestState`
- [ ] 1.2 `packages/core/src/page/page-file.ts` + `page-file.test.ts`：后缀识别、显示名双向转换、
  页面文档解析/序列化、空白页面文档工厂（复用 `createDefaultCanvasSettings` /
  `createDefaultOutputSettings`）
- [ ] 1.3 `packages/core/src/page/page-graph.ts` + `page-graph.test.ts`：
  `readComposePageReference`、深度上限常量、`resolveComposePageNestState`
  （自环、间接环、深度边界 7/8/9）—— 先落定协议，消费者在 `add-page-node-property`
- [ ] 1.4 `packages/core/src/page/index.ts` 与 `packages/core/src/index.ts` 转导
- [ ] 1.5 新包骨架 `packages/pages/`：`package.json`、`tsconfig`、构建与 dts 配置，对齐
  `packages/stage-engine` 的现有配置形态
- [ ] 1.6 `packages/pages/src/page-catalog.ts` + 测试：递归列举、页面过滤、描述符生成、排序
- [ ] 1.7 `packages/pages/src/page-store.ts` + 测试：读写、`createPage`、文档缓存与并发合并、
  `AbortSignal` 透传、`expectedRevision` 冲突与 `force`、`provider.subscribe` 桥接与缓存失效
- [ ] 1.8 **纯重构提交（行为不变、测试全绿）**：抽出
  `packages/asset-browser/src/asset-browser/use-name-prompt.ts` 命名对话框状态机与
  `asset-context-menu.tsx` 菜单内容，内建新建/重命名继续走同一状态机
- [ ] 1.9 `asset-browser-types.ts`：`ComposeAssetContextMenuContext`、
  `ComposeAssetNamePromptRequest`、`ComposeAssetContextMenuItem`、
  `ComposeAssetEntryRenderContext` 与 `contextMenuItems` prop（含 TSDoc）
- [ ] 1.10 `asset-context-menu.test.tsx`：宿主项排序、可见/禁用、`promptName` 确认与取消及名称
  校验、`refresh` 重新列举
- [ ] 1.11 `packages/editor/src/pages/use-page-workspace.ts`：Store 派生、目录订阅、页面会话 Map、
  打开/dirty/保存/冲突/关闭确认
- [ ] 1.12 `workspace-context.tsx` / `workspace-layout.ts` / `workspace-panels.tsx` /
  `workspace-tab.tsx`：`ComposePageDocumentSession`、`pageDocument` 面板 id 与组件、脏点与关闭按钮
- [ ] 1.13 `packages/editor/src/pages/active-page-controller.tsx`：按 `pageKey` key 化的 controller 宿主
- [ ] 1.14 `compose-editor.tsx`：`pages` 配置、`handleAssetOpen` 分流、
  `activeController = pageController ?? props.controller` 构建工作区插槽
- [ ] 1.15 `packages/editor/src/pages/page-context-menu.tsx`：创建页面项 + `editor-i18n.ts` 文案
- [ ] 1.16 `app/src/demo-asset-provider.ts` 增 `Pages/` 目录与页面 `mediaType`；
  `app/src/StageDemo.tsx` 接线 `pages`
- [ ] 1.17 `page-workspace.test.tsx`：双击开页面面板、dispatch 后出现脏点、保存清除脏点、
  冲突弹覆盖对话框、关闭确认
- [ ] 1.18 Playwright：右键创建页面 → 打开标签 → 拖动实体 → 保存 → 重开可见持久化几何
- [ ] 1.19 `bun run lint && bun run typecheck && bun run test && bun run build`

## 2. P2 首页

- [ ] 2.1 `packages/core/src/page/page-manifest.ts` + `page-manifest.test.ts`：宽容解析、未知字段
  保留与写回、`setComposeAppManifestHomePage` 幂等、四类 issue
- [ ] 2.2 `page-store.ts`：`readManifest`、`setHomePage`（惰性创建）、`canWriteManifest`，
  清单写冲突重读重试一次；补测试
- [ ] 2.3 `asset-browser`：`renderEntryBadge` prop，文件树行与目录网格块双处渲染
- [ ] 2.4 `asset-browser` 测试：标记在树与网格双处渲染、返回空结果时不产生额外元素、
  标记不参与命中测试
...
- [ ] 4.1 `editor-controller/controller.tsx`：`runtime` 变化时重置会话状态并重建
