...
- [x] 5.1 文档标签条从画布列头搬进应用顶栏；标签条横向滚动，不挤走右端两段
- [x] 5.2 标志成为回页面库的入口，应用菜单保留（接了端口时标志与 `▾` 是两颗按钮）；
      **菜单里同时留一条「返回页面库」**——标志是快捷入口，不该是唯一入口
- [x] 5.3 画布列头改为文档面包屑。**「设计 / 动画」切换器不做**：那颗已随「动画编辑开关」
      删除，原需求那半句是过期的文字，照抄它等于把那次拆分原样撤销（见 proposal 与 delta）
- [x] 5.4 页面库作为 body 的一种状态；该状态下不渲染工作区切换器。库压在 Dockview 之上、
      被盖的那一层走 `inert`；接了端口即从库开始（不自动打开首页）
- [x] 5.5 保存成功后异步渲染激活场景并上传缩略图，失败只出诊断
- [x] 5.6 更新受影响的既有用例（顶栏段数、标签条位置、选中画法三处）
- [x] 5.7 示例应用用 `?library` 接上端口与演示渲染器：默认关着，否则每一条以「打开就在画布上」
      起手的既有端到端用例都要先经过一屏页面库

## 6. 验证

- [x] 6.1 `bun run lint` / `typecheck` / `test` / `build`
- [x] 6.2 `bun run test:e2e`（顶栏与标签条改了位置）
- [x] 6.3 `openspec validate add-page-library-home --strict`
