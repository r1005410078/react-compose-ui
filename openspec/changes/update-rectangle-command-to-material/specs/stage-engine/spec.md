...
`RECTANGLE` MUST 返回一个盒而不是对角线，`CIRCLE` MUST 返回整圆而不是半径线。

预览与提交 MUST 走同一种效果表示：`RECTANGLE` 两处都用 `boxes`。让预览回折线、提交回盒会
产生一处只有实现者知道的不对称，而覆盖层要为它多认一种形状。
...
- `RECTANGLE`（别名 `REC` / `R`）取两个对角点，产出一个**盒**（`boxes` 效果）而不是曲线。
  判据是**用户画完之后想对它做什么**：画一个矩形外框，下一步九成是给它填色、调圆角、往里塞
  东西——这些 `Curve` 折线全都做不到或做得很别扭。「矩形是四顶点的闭合多段线」那条判断管的是
  `Curve` 协议内部要不要另立 `rect` 类型，它 MUST NOT 被读成「`RECTANGLE` 命令必须产出
  `Curve`」。真需要一个纯几何四边形轮廓时，`PLINE` 画四点按 `C` 就是，不缺入口。
  引擎 MUST NOT 认识 Preset id：`boxes` 只说出「这一步产出一个这么大的盒」，挑哪个物料由
  持有 Registry 的宿主决定——与 `wire` / `arrow` 是同一条既有边界。
