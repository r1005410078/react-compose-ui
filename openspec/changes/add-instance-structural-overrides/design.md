...
命中索引为实例内部实体建立带复合地址的条目。默认单击选中实例整体，进入实例内部需要显式手势
（双击逐层下钻，或修饰键直选），与 Scene Tree 展开/选中状态双向同步。退出下钻恢复到实例整体选区。
...
- 下钻手势的默认绑定（双击 vs 修饰键）需在实现期结合现有 Stage 手势状态机确认，避免与既有平移/框选冲突。
