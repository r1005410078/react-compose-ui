...
- 当字段有多个 targets，或绑定与存在性、重置、集合操作竞争默认操作槽时，操作列显示一个聚合入口。
- 聚合入口按“绑定”与“其他操作”分区；绑定区按 descriptor 顺序列出 target label、当前变量和状态。
  选择 target 后打开现有变量选择器，返回时焦点回到聚合入口。
...
规划函数保持纯逻辑，以默认三槽、窄列、单/多目标、bound/invalid、重置和 disabled 组合做单元测试。
不能通过让按钮溢出、撑宽列或覆盖编辑区来容纳入口。
