...
- 目标：绑定与现有行操作在默认双图标操作列中确定地共存，并让重置保持可见。
- 目标：默认三列布局优先保障编辑器宽度，而不牺牲双图标操作轨道。
...
操作列默认保持 38px，因此显隐不会改变字段、编辑器或相邻行的几何。绑定后的原输入不再渲染，
...
决定直接入口、聚合入口与溢出内容。默认操作列宽度恰好容纳两个 16px 图标（含 2px 间距与边距），拖宽到 56px 起可用第三槽；不足
...
规划函数保持纯逻辑，以默认两槽、窄列、单/多目标、bound/invalid、重置和 disabled 组合做单元测试。不能
通过让按钮溢出、撑宽列或覆盖编辑区来容纳入口。

默认 Inspector 内容区约为 365px：属性名列采用 120px、操作列保持 38px，因此中间编辑列默认获得约 207px。
