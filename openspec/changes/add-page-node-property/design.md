...
- **画布实时嵌套渲染的风险面比预估窄**（实现后订正）：`data-entity-id` 只由 Stage 自己的
  scene layer 产出，而 `createStageSceneIndex` 只遍历外层文档的 `rootIds` 与 `Hierarchy`。被引用
  页面的实体属于**另一份文档**，因此框选、吸附候选与场景树投影结构上就取不到它们 —— 这是
  by construction，不需要逐项维护，但仍加了回归断言锁住。实际需要处理的只有「嵌套 DOM 抢
  指针事件」，由编辑态 `pointer-events: none` 解决，红灯测试已覆盖。
- **嵌套内容容器必须自带尺寸**：嵌套实体是绝对定位的，容器一旦塌陷成零高度，元素存在但用户
  什么也看不到。尺寸内联给定，不依赖样式表加载顺序。这个问题只有 e2e 能发现。
