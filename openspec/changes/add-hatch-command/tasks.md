...
- [x] 4.6 预览与落地同一块面；两支的提示文案不同；断口画在走不下去的节点上（端到端，因为这三条只在命令行与覆盖层上可观察）
...
- [x] 6.1 圆压在矩形上 → 填矩形内圆外那块 → 落成 `path`，两个边界对象不动
- [x] 6.2 点没有东西穿过的矩形 → 矩形自己被填上，文档不新增 Entity
- [x] 6.3 线穿过矩形 → 点半边 → 新建一个填充对象，矩形仍空心
- [x] 6.4 岛：矩形内有圆 → 挖空
- [x] 6.5 不封闭 → 拒绝 + 断口记号；补上那一段后再点即成
- [x] 6.6 层序：填充压在边界与区域内的符号之下
- [x] 6.7 换色：工具栏色板与 `C` 关键字各一条
- [x] 6.8 用例 MUST 在非 100% 缩放下断言（`world = (屏幕 − 视口) / zoom`）
