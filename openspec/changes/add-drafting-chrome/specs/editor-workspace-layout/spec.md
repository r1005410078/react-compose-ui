...
**「画线」的主按钮 MUST 是 `PLINE` 而不是 `LINE`。**两条命令都保留（与 AutoCAD 一致），但
默认入口给产出**一个** Entity 的那一条：在页面编辑器里一个 Entity 就是场景树的一行、动画的
一个目标、脚本绑定的一个名字，画一根四折的导线得到四行是 AutoCAD 不用付的代价。`LINE` 退成
「两点直线」，仍可从工具栏与命令行进入。
...
#### Scenario: 画线的主入口产出一个 Entity

- **WHEN** 用户点「画线」主按钮并连取四个点
- **THEN** 文档里只增加**一个** Entity
- **AND** 场景树里只多一行
