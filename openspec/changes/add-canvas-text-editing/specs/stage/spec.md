...
以文字工具**点击**创建的文字 MUST 以空内容进入编辑，MUST NOT 保留 Preset 的占位文案——占位文案会
逼用户先全选删除再打字。拖拽创建的固定尺寸文字不受此约束。

`draw-text` 的绘制预览 MUST NOT 显示占位文案，只保留光标提示：预览里的文字等于承诺一段并不会存在
的内容，松手后它会立刻消失，看起来就是闪了一下。
...
#### Scenario: 绘制预览不显示占位文案

- **WHEN** 用户以文字工具在画布上按下并保持
- **THEN** 预览只显示边框、尺寸标签与光标，不出现任何占位文案
- **AND** 松手后画布上不出现一闪而过的文字
