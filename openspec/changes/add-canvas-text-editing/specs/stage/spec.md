...
`draw-text` 的绘制预览 MUST NOT 显示占位文案，也 MUST NOT 显示尺寸标注，只保留光标提示：预览里的
文字等于承诺一段并不会存在的内容，松手后它会立刻消失，看起来就是闪了一下；而尺寸标注会暗示一个
用户根本改不了的数字。
...
#### Scenario: 绘制预览不显示占位文案与尺寸

- **WHEN** 用户以文字工具在画布上按下并保持
- **THEN** 预览只显示边框与光标，既不出现占位文案也不出现尺寸标注
