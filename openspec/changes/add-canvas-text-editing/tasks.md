...
- [x] 3.1 `pointer.down` 增加连击计数字段，surface 按平台惯例归一化后传入
- [x] 3.2 context 增加「Entity 是否可原地编辑」判定入口与新建 Entity 回灌字段
- [x] 3.3 红测：连击计数 1 走选择/移动，计数 2 进入会话；不可编辑 Entity 双击不进入
- [x] 3.4 红测：回灌 `draw-text` 新建 Entity 后进入会话，且重复 context 更新只进入一次
- [x] 3.5 红测：其他绘制工具创建时不进入会话
- [x] 3.6 红测：会话期间指针拖拽不产生移动/缩放/旋转/框选命令
- [x] 3.7 红测：`Esc`、目标外按下、选区变化、目标消失都退出会话
- [x] 3.8 `StageInteractionController` 实现会话判定与 effect，保持无 DOM
- [x] 3.9 单选可编辑 Entity 时 `Enter` 进入编辑，接入既有快捷键动作表

## 4. Stage 原地编辑与覆盖层

- [x] 4.1 红测：编辑态不渲染八向与旋转手柄，只渲染编辑边框
