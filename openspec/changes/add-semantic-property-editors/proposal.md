...
- 在 `@compose-ui/property-panel` 发布默认可用的语义 editor：`vector2`、`size`、`angle`、`opacity`、`corner-radius`、`stroke-width`、`visibility`、`color`、`alignment`；实例 renderer 可以按相同 ID 覆盖默认实现。
...
- 将 Canvas Inspector 的输出宽高与背景分别迁移到 Size 和 Color，不改变 `output.configure` 命令、文档 Schema 或持久化数据。
...
- Public API: 新增稳定 editor ID、Size preset metadata、renderer 字段显示名与 `ComposeColorPicker`；不破坏既有受控属性面板 API。
