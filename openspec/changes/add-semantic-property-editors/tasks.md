...
- [x] 2.2 实现 Vector2、Size、Angle、Opacity、Corner Radius、Stroke Width、Visibility、Color、Alignment，并接入现有校验、只读、重置和绑定。
...
- [x] 3.2 将 Canvas Inspector 迁移为 Size + Color，去除独立尺寸属性行和局部 preset renderer。
...
- [ ] 4.2 运行 `openspec validate add-semantic-property-editors --strict`、受影响包测试/构建、仓库 lint/typecheck/test/build/test:e2e 与 `git diff --check`；在每项任务下补全 Red/Green/Regression 证据。
  - Passed: `bunx openspec validate add-semantic-property-editors --strict`、`bun run lint`、`bun run typecheck`、`bun run build`、`git diff --check`；components（23）、property-panel（61）、materials（21）及 editor（64）测试通过。
  - Scoped browser regression: Canvas Color Picker、SVG Material Inspector、完整 Stage 纵向流程及两条网格吸附流程均通过；本次更新了受语义编辑器影响的黄金图。完整 Playwright 为 11 通过、3 个与 Color Picker 无关的失败（高速 pointer 手势、组合 Frame 直接拖动、设置弹框 inert）。
  - Repository test blocker: `bun run test` 的 Storybook Button 有 4 个既有色彩对比 a11y 失败；该浏览器子进程在报告失败后未自行退出，3 分 29 秒后手动停止。未涉及本 change 的包或文件。
