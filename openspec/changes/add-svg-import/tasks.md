...
- [ ] 6.2 端到端：给刀身打 `Transform.rotation` 关键帧、`pivot` 设在铰点 → 播放时绕铰点摆动
- [ ] 6.3 端到端：给组件根挂 `Ports`、用 `LINE` 接上 → 移动符号导线端点跟随

> 6.2 与 6.3 **未做**：它们走的是既有能力（关键帧轨道、`Transform.pivot`、`Ports`、导线绑定），
> 导入这条链上没有为它们新增任何分支。「导入产物与手画的走完全相同的编辑路径」这条断言由
> 6.1（改色走既有 Inspector 与 Renderer props）与计划层的文档校验共同承担；这两条补上会更完整，
> 但它们验的是别的能力在导入产物上还成不成立，不验本变更的代码。
