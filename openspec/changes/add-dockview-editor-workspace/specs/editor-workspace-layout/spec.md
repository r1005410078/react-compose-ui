...
系统 MUST 将 `ComposeEditor` 的 `children` 渲染在 Canvas 内容区域，并将 `sceneGraphPanel`、`canvasToolbar`、`inspectorPanel`、`transactionLogPanel`、`commandPanel` 分别渲染在对应语义区域。缺少任一可选插槽时，系统必须在对应区域显示可访问的占位内容。

#### Scenario: 宿主提供全部工作区内容

- **WHEN** 宿主同时传入 `children` 和五个命名插槽
- **THEN** 每份内容显示在对应语义区域中

#### Scenario: 宿主省略可选插槽

- **WHEN** 宿主省略任一命名插槽
- **THEN** 对应区域显示说明该区域用途的占位内容

#### Scenario: 插槽内容更新

- **WHEN** 宿主在编辑器挂载后更新任一插槽的 React 内容
- **THEN** 对应区域显示最新内容
- **AND** Dockview 组、面板和用户调整后的尺寸不被重建

### Requirement: 嵌入与公共 API 边界

系统 MUST 保留 `ComposeEditor` 根 `<section>` 的标准 HTML 属性、默认可访问名称、`data-compose-ui="editor"` 和 core 包标识。系统不得从 `@compose-ui/editor` 公共入口导出 Dockview API、序列化类型、Edge Group 对象或内部面板对象。
...
- **AND** 不需要导入或引用任何 Dockview 类型

### Requirement: 编辑器样式入口

系统 MUST 提供 `@compose-ui/editor/styles.css` 样式入口，其中包含 Dockview 所需基础样式和作用域限定的深色工作区样式。系统必须在包文档中说明宿主需要导入该样式并为编辑器提供非零高度。
