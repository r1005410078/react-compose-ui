> **注意**：以下勾选与命令记录来自已删除的分支 `feat/add-linked-component-library`
> （`135f8f4`，2026-08-03），**不代表 main 当前状态**。main 上没有对应实现，重新实施时
> 需以当前 main 为基线重跑全部步骤。详见 `proposal.md` 的「状态」一节。

## 1. 规范与 Core

- [x] 1.1 定义 Component Document v1、引用、暴露属性、覆盖与嵌套校验
...
- [x] 2.1 新增目录、Store、缓存、订阅、取消和冲突测试与实现
...
- [x] 2.2 新增聚合基础 Preset 与项目组件的受控面板
...
- [x] 3.1 新增 component-instance Asset Drop、快照渲染、显式覆盖与嵌套保护
...
- [x] 3.2 接入保存子树、独立标签、属性暴露、更新提示和单事务更新
...
- [x] 4.2 运行 lint、typecheck、test、build、test:e2e 与严格 OpenSpec 校验
