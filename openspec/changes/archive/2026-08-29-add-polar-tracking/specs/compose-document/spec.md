...
**原因**：管线次序被本变更**改了**——角度约束从「捕捉之前」挪到了**网格之后**，成为最后一步。
旧要求的两条场景因此连名字一起作废：「指针取点依次经过正交与网格」说反了顺序，
「捕捉命中后不再经过正交与网格」漏掉了新加进来的那一档。整条移除、以新名新增，而不是
MODIFIED：MODIFIED 只能补充与改写，改掉场景的名字需要显式说出来。

**迁移**：见新需求「共享点输入管线与角度约束」。「键入的坐标不被任何吸附改写」这条不变量
原样保留。
