...
**原因**：这条要求的核心是「场景背景 MUST 与 Container Preset 默认外观相同」，连同它的
「新建场景与新建容器同底色」场景一起被本变更推翻——场景背景是会被发布出去的真实像素，由用户
决定用什么底。整条移除、以新名新增，而不是 MODIFIED：MODIFIED 只能补充与改写，丢掉一条已经
不成立的场景需要显式说出来。

**迁移**：见新需求「场景默认外观独立于容器」。边框宽度为 0 与校验探针那两条约束原样保留。
