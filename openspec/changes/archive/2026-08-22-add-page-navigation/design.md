...
- 决策:`navigate.target` 允许为 `null`。
  属性面板的数组编辑器"添加项"由 Schema 构造默认项,用户随后才能在那一行里挑页面;
  不允许 null 会让"新建一条交互"与"选好目标"互为前提。Page Slot 的 `page: null` 是同一个
  先例。**不完整**的引用仍然非法——"配错了"与"还没配"必须可区分。
