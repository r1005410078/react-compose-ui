...
Version 1 MUST 保持既有 `rendererProps.fields` 顶层 Prop 绑定，且 MUST 只允许出现在拥有 Renderer 的
Entity 上；version 2 MUST 允许可选 `rendererProps` 与 `componentFields`，其中 Component Key、稳定
Field 名称分别映射到 `{ scope: 'page', exportName: string }`。`rendererProps`（version 1 与 2）MUST
只允许出现在拥有 Renderer 的 Entity 上；`componentFields` MUST NOT 要求 Entity 拥有 Renderer，只要求
Entity 上存在对应 Component。Bindings MUST 只保存严格 JSON 引用，不得保存脚本当前值、State、Computed
或 Function。Core MUST 校验引用形状但 MUST NOT 依赖运行时 Registry 判断 Renderer Prop、Component 或
...
- **WHEN** Animation Component 的 `play` 字段绑定到页面返回成员 `playFault`
- **THEN** Core 以 version 2 componentFields 保存 Component Key、稳定 Field 名称和页面引用
- **AND** Animation authored play 与 playFault 当前 boolean 值分别保留且互不覆盖
...
- **THEN** Core 与 Renderer 绑定 Runtime 保持既有解析结果
- **AND** 只有首次保存 Component 字段绑定时才把候选写成 version 2
