...
MUST NOT 超过 90°（生成侧每段至多 90°，更长的弧一定是两段）；再**在曲线上取样**，每个样点到
圆心的距离与半径之差 MUST 都在容差内，且样点的方位角 MUST 都落在这段扫掠之内。
...
方位角落在扫掠之内这一条挡的是**折返**：三次段冲过终点再绕回来时，轨迹是圆的一段但不是这段
弧，按弧参与运算会少掉冲出去的那一截。
...
**矢高小于量化步长的弧 MUST 认成直线段而不是弧**：那样的弧在存储精度下就是一条直线，它的
半径已经不可复原，而按直线参与运算带来的形状误差不超过量化本身。这一条由「先试直线」的次序
自动成立——判据是同一个容差。

#### Scenario: 生成的弧段认回同一段弧

- **WHEN** 半径 2 到 500、扫掠 15° 到 90°、两个方向的弧经闭式解转成三次段并舍入到两位小数
- **THEN** 每一段都被识别为弧，且认出来的弧与原弧在整段上处处吻合到容差之内

#### Scenario: 矢高小于量化步长的弧认成直线
...
#### Scenario: 冲过终点再折返的段不认

- **WHEN** 一条三次段的轨迹落在某个圆上，但它冲过终点之后又绕了回来
- **THEN** 它不被识别为弧
