...
- [x] 2b.2 `curve-arrangement.ts`：`NODE_EPSILON_FLOOR = 2 × 10^-COMPOSE_GEOMETRY_PRECISION`；
      `resolveComposeCurveBoolean` 取 `max(下限, 1e-7 × 尺度)`。**只动布尔**——`HATCH` 求面的
      「没有间隙容差」是明写的产品决定

      > 证据是**端到端**而不是单测，这是有意的取舍。量过的：去掉下限，浏览器里「一个圆 + 由它
      > 围出来的填充求并集」与「框选全部求并集」立刻回到「求解退化」；补上就通过。而构造出来的
      > 另一组数值（圆心 0 与 137、半径各 100）即使补了下限仍然退化——近似共圆的边还没有归一，
      > 那是另一次变更。因此**不写一条声称「这一类都能算」的单测**：它要么过度承诺，要么把一个
      > 靠数值运气成立的结果钉死。规范里同样只承诺容差本身，并把这处脆弱写在明处。
      >
      > 中间写过两条想当判别性用例的：一条在共边矩形上（T 形交点与共线合并本来就兜住了，加不加
      > 下限都绿），一条在构造的月牙上（加了下限也红）。两条都删掉了——一条永远绿、一条永远红，
      > 都不说明容差这一处。
