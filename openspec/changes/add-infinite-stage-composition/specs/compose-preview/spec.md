...
`@compose-ui/preview` MUST 接受 ComposeDocument、ComponentRegistry 与显式 frameId，并使用普通 DOM
按文档层级渲染目标 Frame。Preview MUST 依赖 core 与 component-registry 公共入口，不得依赖
editor、stage、scene-tree 或 property-panel。

#### Scenario: 预览指定 Frame

- **WHEN** 宿主提供包含多个 Frame 的合法文档、registry 和有效 frameId
...
- **AND** 不渲染 selection、handles、guides、grid 或其他编辑状态
