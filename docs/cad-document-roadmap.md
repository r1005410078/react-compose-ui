...
**当前状态：步骤 1–5 与 6a 已完成。** 本文写于 2026-08-21，记录的是动手前的判断与决策，
每完成一步回填实测结果与被推翻的预判。
...
#### 6b · 对象捕捉 — `add-cad-osnap`（未开始）

需要查询既有几何的部分：端点 / 中点 / 交点捕捉、捕捉标记，以及它们依赖的 CAD 空间索引。
选择集与「先选后执行」同刀落地——**选择集才是仲裁器真正开始有价值的地方**。
