...
### 步骤 8 · 连接语义 — `add-cad-ports-and-wires` ✅ 已完成
...
三样地基都已落地，其中第三样的形态与预想不同：

- 符号声明端口：`CadBlockDefinition.ports`，块局部坐标，跟随实例变换。
- 连线端点存 `{entityId, portId}`：`CadWire` 的端点是自由点或端口引用。
- **「移动或切换变体时重解端点」不需要写**：几何求解不存储，因此没有会过期的东西。存坐标
  的方案要在每条改变实例位置的路径上挂重解钩子，而漏掉一条的症状是「线错位」——看起来像
  渲染 bug，排查方向从一开始就是错的。

「变体切换要求端口 id 稳定」立成了**校验器**里的不变量而不是约定：`wire.unknown-entity` /
`wire.not-instance` / `wire.unknown-port` 三个可判别机器码。写成命令级检查只挡得住写过检查
的那一条路径，而改 `blockId` 的路径不止一条（命令、外部写入、DXF 导入）。

顺带修掉一处此前看不见的分叉：拖动预览原先按屏幕像素平移已渲染线段，导线一进来必然错——
拖一台设备时它的导线并未被选中。改为对预览文档跑一次与提交完全相同的 `translateCadEntity`，
预览与提交因此不是「一致」而是同一段代码。

正交走线（曼哈顿路由）、避障、线段合并是打磨层，可后置；多段导线要与自动路由一起做，单独
加中间拐点只是把维护线形的负担交回给用户。

### 步骤 8.75 · 圆与圆弧 — `add-cad-curves` ✅ 已完成

路线图原先没有这一步，但「图元只有两点直线」是三类目标场景共同的硬缺口：一次接线的断路器与
接地、原理图的引脚圆点、拓扑图的节点圆，主体都是圆与弧。它同时挡着步骤 9——DXF 里最常见的
两类实体就是 `CIRCLE` 与 `ARC`，模型表达不了就只能在导入时降级成折线，而降级不可逆。

落地形态：一个 `CadArc`（整圆是扫掠 ±360 的弧），可见性遍历改为**几何联合**，`CIRCLE` 与
三点定弧的 `ARC` 两条命令，圆心与象限点两种捕捉。

**遍历返回联合而不是把弧拍扁**是本步唯一的架构决定。拍扁能让下游一行不改，代价是渲染要么
画出可见的多边形、要么绕开遍历而与命中分叉；而且圆心与象限点在拍扁之后**不存在**——接线时
想捕的恰恰是这两个点。

过程中修掉一个此前一直存在的缺陷：提交点时用的捕捉来自上一帧 hover，`pointerdown` 赶在 React
重渲染之前到达时，落点会被吸回用户已经离开的特征点。画圆时的症状是「半径不能为零」。
...
- `LWPOLYLINE` 的 bulge 是圆弧段，不是直线——步骤 8.75 之后已有 `CadArc` 可以落。
