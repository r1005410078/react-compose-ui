...
> **2026-08-22 起本路线已完结并被接替**：步骤 1–9 全部完成后，产品方向修订为「CAD 画法
> 融入页面世界、独立文档外壳拆除」，后续方向见
> [`drafting-unification-roadmap.md`](./drafting-unification-roadmap.md)。本文保留为已完成
> 各步的决策与实测记录——其中的图元数学、捕捉、命令行与 DXF 结论在新路线中原样存活。
...
### 步骤 8.9 · 文字 — `add-cad-text` ✅ 已完成

图元里真正**做不到**（而不是麻烦）的只有文字：矩形可以用四条线或一个块凑，折线可以连着画，
而没有任何既有图元能表达一串字符。三类目标场景都以「谁是谁」为主要信息，一张没有标注的
接线图交不出去。

落地形态：`CadText`（锚点 / 内容 / 字号 / 旋转 / 对齐），`TEXT` 三步命令，`insertion` 捕捉，
以及把遍历的返回类型从 `CadCurve` 抬成伞类型 `CadGeometry`。

两个值得记住的判断：

- **`CadCurve` 没有被弄脏。** 线段与圆弧共享一整套数学，文字与它们一个共同运算都没有。加一层
  伞比把一个已经准确的名字改成「凡是画得出来的东西」便宜。
- **文字按包围盒命中不是破例。** 「按距离而不是包围盒」的理由是「对角线的盒子里大部分是空的」，
  而文字占满自己的盒子。把它的「距离」定义成框内 0、框外到框边，既有的命中仲裁与框选判定
  一行不改——规则没变，只是它对不同几何给出了不同的、正确的答案。

字宽需要字体度量而 `cad` 无 DOM，因此图面用等宽字体、包里用固定比例算框。这条耦合是不变量：
换字体必须同时换比例。

### 步骤 8.95 · 多段线与矩形 — `add-cad-polyline` ✅ 已完成

画一个设备框此前要连点四下 `LINE`，得到四个互不相干的图元——随后每一次移动、删除、框选都要
为此付账。矩形是拓扑图上频率最高的形状，让它成为**一个对象**不是省几次点击，而是让所有以
「对象」为单位的操作重新成立。

落地形态：`CadPolyline`（顶点 + `closed`），`PLINE` 与 `RECTANG` 两条命令。矩形是四顶点的
闭合多段线，不另立类型。

**遍历把多段线展开成线段，这与上一刀拒绝「拍扁圆弧」不矛盾。** 判据是展开会不会丢东西：
圆弧拍扁后形状变成可见的多边形、圆心与象限点消失；多段线本来就是一串线段，顶点仍是各段
端点、各段仍有中点，形状逐像素相同——展开是恒等变换而不是近似。因此命中、框选、捕捉与渲染
四条路径一行都没改。

`bulge` 刻意没做：现在没有命令产出它，位置已经让好了——步骤 9 的导入器只需把某一段从
`segmentCurve` 换成 `arcCurve`。

### 步骤 8.4 · 图元外观覆盖 — `add-cad-appearance` ✅ 已完成

步骤 8.5 的**前置**。路线图早就写明：CAD 动画的主要对象不是几何而是外观与状态，而图元此前
一点外观都没有——颜色一律 ByLayer、线宽写死 1、没有线型。动画没有可动的属性。

它本身也是独立缺口：接线图靠线型区分一次/二次回路、靠颜色区分电压等级、靠线宽区分主干与
支路，只能按图层区分意味着「换一根线的颜色」要先新建一个图层。

落地形态：`CadStroke`（三个字段**各自可选**，缺席即回退），`resolveCadStroke` 单一读取入口，
`COLOR` / `LWEIGHT` / `LTYPE` 三条共用一个会话工厂的命令。

两个值得记住的判断：

- **清除即删键**，不写哨兵。JSON 没有 `undefined`，写 `null` 会让「显式设成无」与「跟随图层」
  变成两个都要处理的状态，而那个分支要出现在每一处读外观的地方。
- **线宽是屏幕像素、虚线是世界单位**。看起来不一致，其实是照抄两个不同的领域概念；两处
  `zoom` 一个有一个没有，这条要立在注释里。

`dashOffset` 刻意没做——它只有被动画时才有意义，步骤 8.5 里才有真实的消费者。

### 步骤 8.5 · CAD 动画 — `add-cad-animation` ✅ 已完成
...
**前置依赖已由步骤 8.4 解决。** `CadStroke` 提供了 per-entity 的颜色、线宽与线型；`dashOffset`
留给本步添加——`CadStroke` 加一个可选字段不牵动别的东西。而 CAD 动画的主要对象**恰恰不是几何而是外观与状态**：
...
- **协议、采样、插值一行没改。** `applyComposeAnimationAtTime` 的类型参数放宽到「任何带
  `entities` 的文档」，实现一个字符没动——它本来就只遍历 `entities`。这不是为复用而扭曲一个
  函数，是把它已经具备的通用性写进类型。
- **清单落在文档级**，比页面那套更简单。轨道的归属照抄页面（住在 Entity 上），因此 `ERASE`
  删掉一根导线时它的轨道跟着消失，不需要补偿代码。
- **采样只作用于渲染**，命中与捕捉继续读作者文档。因此 v1 只动外观：几何一动不动，不存在
  「看得见却点不中」。几何轨道要等「动画期间怎么编辑」有答案之后再谈。

`FLOW` / `FL` 是第一条也是唯一一条作者入口：选中导线即得到一根流动的线。它顺带补上默认虚线
——`stroke-dashoffset` 只对虚线有意义，交付一个没有视觉效果的动画等于没交付。

**「变体切换能不能表达成轨道」仍然未决**，本步不猜：它是结构变更而不是数值插值，需要一种
step 轨道。时间线面板、动画文件与脚本绑定同样留给后续。

### 步骤 9 · DXF 导入 — `add-dxf-import` ✅ 已完成
...
- **Y 轴翻转**：落成一个矩阵恒等式的三个推论（位置翻转、旋转取反、比例不变），而不是三条各自
  记着的规则。拦它的用例用一段**不对称**的弧——对称几何在翻转错误下看起来仍然正确。
- **`LWPOLYLINE` 的 bulge**：本步按弦导入并给出诊断。导入器自己解决不了它——`CadPolyline`
  没有 bulge 字段，拆成「多段线 + 独立圆弧」会让它不再是一个对象。真要支持，该改的是协议与
  那条遍历，属于另一刀。
- **`LAYER`**：这条已经过期了——步骤 4 之后 `CadDocument` 本来就有图层表，直接按名字映射即可。
- **`$INSUNITS`**：**不需要 fit 变换**。CAD 是无限图纸、没有图纸比例，1 个图形单位就是 1 个
  画布单位；真正缺的是**打开时按内容取景**，那是画布的事而不是单位的事。这条当初判断错了方向。

本步还确认了一件事：**诊断是这个功能的一半**。真实 DXF 一定带着表达不了的东西，静默丢弃会让
用户认为工具不可靠，整份拒绝会让一个 `DIMENSION` 毁掉 95% 可用的图。
