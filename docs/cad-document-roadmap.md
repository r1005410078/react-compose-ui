...
**当前状态：步骤 1–5、6a、6b 已完成。** 本文写于 2026-08-21，记录的是动手前的判断与决策，
每完成一步回填实测结果与被推翻的预判。
...
#### 6c · 选择集与手势仲裁 — `add-cad-selection`（未开始）

点选、框选、Esc 清除选择，以及「先选后执行 / 先执行后选」的语义。**这里才是仲裁器真正开始
有价值的地方**：点选、框选与命令取点三者第一次真正竞争同一个指针，`activate(command)` 该长
什么样也要到这时才看得清（见步骤 5 的说明）。
