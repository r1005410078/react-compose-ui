...
import type { TreeItemAdapter } from '@compose-ui/components'
import {
...
const assetTreeAdapter: TreeItemAdapter<AssetTreeEntry> = {
          ...
          {selectedEntries.length > 1 ? (
            ...
            <div aria-label={folder?.name} className="asset-browser__grid" role="grid">
              {source.loading.has(folder?.id ?? '') ? (
                <div className="asset-browser__status">{messages.loading}</div>
              ) : visibleFolderChildren.length === 0 ? (
                <div className="asset-browser__status">{messages.emptyFolder}</div>
