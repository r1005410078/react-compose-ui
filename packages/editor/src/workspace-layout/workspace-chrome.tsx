...
/**
 * 应用顶栏上的文档标签条：`tablist`，紧接标志之右。
 *
 * @remarks
 * 它住在顶栏而不是画布列头，因为**页面库那一屏没有画布列**：标签住在那里意味着一进页面库标签
 * 就消失——而标签正是「我手上开着哪几张图」，去库里找下一张时最需要它。标志与已开的文件也是
 * 同一排去处（回库与切到另一份图都是「去哪儿」），分两行会让两个同类动作住在两个地方。
 *
 * 它不做成 Dockview 组头：那要求文档成为 Dockview panel，而文档不参与拖放、浮动与布局持久化，
 * 正是「文档不是 panel」这条存在的理由。
 *
 * 横向滚动收在内层：标签再多也只压缩自己，右端的工作区切换器与三个布局开关一格都不让
 * （`flex: none`）——顶栏右端是固定的去处，被标签推走就等于按开的文件数量换位置。
 *
 ...
 *
 * @remarks
 * 段与段之间**只有间距**，不画竖线：标志与标签钉在最左、切换器与开关钉在最右，位置本身已经
 * 分好段，而竖线是把「我数得清有几段」画出来，那件事本来不需要用户去数。
 *
 * 顶栏承载**应用与视图作用域**，而「我开着哪几份」在页面库成为入口之后**就是**应用作用域——
 * 它不再是画布那一列的视图状态。保存按钮与模式切换器仍不在这里：它们是文档作用域，分别由
 * `document.save` 动作与画布列头承担。
 *
 * 页面库那一档不渲染工作区切换器：那一屏没有画布，因而没有工作区。标签条与布局开关照常——
 * 标签正是去库里找下一张时最需要的那一样。
 */
export function EditorTopBar() {
  const { libraryOpen } = useWorkspaceContent()
  return (
    <div className="compose-editor__top-bar">
      <EditorBrandMenu />
      <WorkspaceDocumentTabs />
      <div className="compose-editor__top-bar-spacer" />
      {libraryOpen === true ? null : <WorkspaceSwitcher />}
