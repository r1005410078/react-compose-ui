...
it('OpenSpec: history / 独立历史记录包 / 独立消费历史能力', () => {
  expect(ComposeHistoryPanel).toBeTypeOf('function')
  expect(useComposeHistory).toBeTypeOf('function')
  expect(useComposeHistoryShortcuts).toBeTypeOf('function')
})

describe('useComposeHistory', () => {
  beforeEach(() => {
    vi.useFakeTimers()
  ...
  it('OpenSpec: history / 不可变会话时间线 / 提交与逐步导航', () => {
    const { result } = renderHook(() => useComposeHistory({ count: 0 }))
  ...
  it('OpenSpec: history / 不可变会话时间线 / 忽略无变化提交', () => {
    const { result } = renderHook(() => useComposeHistory(
      { count: 0 },
  ...
  it('OpenSpec: history / 不可变会话时间线 / 重置时间线', () => {
    const { result } = renderHook(() => useComposeHistory(0))
  ...
  it('OpenSpec: history / 合并、分支与容量限制 / 合并连续输入', () => {
    const { result } = renderHook(() => useComposeHistory('', { mergeWindowMs: 750 }))
  ...
  it('OpenSpec: history / 合并、分支与容量限制 / 超出容量限制', () => {
    const { result } = renderHook(() => useComposeHistory(0, { limit: 2 }))
    act(() => result.current.commit(1, { label: '一' }))
    act(() => result.current.commit(2, { label: '二' }))
    act(() => result.current.commit(3, { label: '三' }))
  ...
  it('keeps one baseline under Strict Mode replay', () => {
    const { result } = renderHook(() => useComposeHistory('initial'), { wrapper: StrictMode })
    expect(result.current.entries.map((entry) => entry.label)).toEqual(['开始'])
  })
})
...
describe('ComposeHistoryPanel', () => {
  it('OpenSpec: history / 受控历史面板 / 显示并跳转历史', () => {
    ...
    render(<ComposeHistoryPanel controller={controller} />)
  ...
  it('OpenSpec: history / 历史面板内建本地化 / 使用英文历史面板', () => {
    const controller = createNavigationController()
    render(<ComposeUIProvider locale="en-US"><ComposeHistoryPanel controller={controller} /></ComposeUIProvider>)
  ...
  it('OpenSpec: ui-context / 共享 UI Context 包 / 独立组件保持兼容 - History 消费 Context', () => {
    ...
    render(
      <ComposeUIProvider locale="en-US" theme="light">
        <ComposeHistoryPanel controller={controller} />
      </ComposeUIProvider>,
    )
...
function ShortcutFixture({
  ...
  const onKeyDownCapture = useComposeHistoryShortcuts(controller, shortcuts)
  return (
...
describe('useComposeHistoryShortcuts', () => {
