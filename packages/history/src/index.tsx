 ...
 *
 * @remarks
 * 本包不拥有页面文档 Schema，也不执行持久化。传给 `useComposeHistory` 的值必须由宿主以不可变方式
 * 更新；包会保留快照引用并利用结构共享控制内存成本，不会隐式深拷贝或序列化值。
 *
 * @packageDocumentation
 */
export { ComposeHistoryPanel } from './history-panel'
// eslint-disable-next-line react-refresh/only-export-components -- 公共入口同时导出组件与 Hook。
export { useComposeHistory } from './use-history'
// 公共入口需要同时导出组件和 Hook，因此只针对这一行豁免 Fast Refresh 的组件导出限制。
// eslint-disable-next-line react-refresh/only-export-components -- 公共入口同时导出组件与 Hook。
export { useComposeHistoryShortcuts } from './use-history-shortcuts'
export type {
