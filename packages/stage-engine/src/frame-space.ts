 ...
 * 为最近的祖先 Frame；没有选择时才回退到宿主给出的默认 Frame，最后回退到第一个根 Frame。
 * `defaultFrameId` 只承担回退职责，MUST NOT 覆盖显式选择。
 *
 * @public
 */
export function resolveActiveFrameId(
  ...
  if (defaultFrameId && isComposeFrameEntity(document.entities[defaultFrameId])) {
