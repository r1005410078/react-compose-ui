...
describe('Stage ECS commands', () => {
  ...
  it('OpenSpec: stage-engine / ECS 结构命令 / 进入 Auto Layout 容器只改定位方式，不改写轴尺寸', () => {
    // 父级静默改写子级尺寸会让「我明明设了 40 高」在属性面板上读不出原因，因此 reparent
    // 只把 positioning 转成 flow；交叉轴要不要拉伸由 alignItems 在求解时决定。
    // row 与 column 两个方向都断，漏掉任一个都会让「只改了一根轴」蒙混过关。
    for (const [direction, container] of [
      ['row', autoLayoutContainer('container', [])],
      ['column', (() => {
      ...
      const value = document([child, container], ['child', 'container'])
      const runtime = createTransactionRuntime({ document: value })
      runtime.dispatch(createReparentCommand(
        value, layoutSnapshot(value), ['child'], 'container', 0,
      ))
      const item = getComposeLayoutItem(runtime.document.entities.child!)
      expect(item.positioning, direction).toBe('flow')
      expect(item.width, direction).toMatchObject({ mode: 'fixed', value: 80 })
      expect(item.height, direction).toMatchObject({ mode: 'fixed', value: 40 })
    }
  })
