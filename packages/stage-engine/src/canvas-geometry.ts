 ...
 *
 * @remarks
 * 视觉刻度至少相隔 8px，文字至少相隔 48px；抽稀不会改变实际网格吸附刻度。
 *
 * @public
 */
export function createRulerTicks(options: {
  ...
  const { axis, viewport, length, step, offset, primaryLineEvery } = options
  let stride = 1
  while (step * stride * viewport.zoom < 8) stride *= 2
  const visibleStep = step * stride
  const screenOffset = axis === 'x' ? viewport.x : viewport.y
  const worldStart = (0 - screenOffset) / viewport.zoom
  const worldEnd = (length - screenOffset) / viewport.zoom
  const firstIndex = Math.floor((worldStart - offset) / visibleStep) - 1
  const lastIndex = Math.ceil((worldEnd - offset) / visibleStep) + 1
  const labelEvery = Math.max(1, Math.ceil(48 / (visibleStep * viewport.zoom)))
  const ticks: StageRulerTick[] = []
  for (let index = firstIndex; index <= lastIndex; index += 1) {
    const value = offset + index * visibleStep
    const screen = value * viewport.zoom + screenOffset
    const gridIndex = Math.round((value - offset) / step)
    const major = gridIndex % primaryLineEvery === 0
    ticks.push({
      value,
      screen,
      major,
      ...(index % labelEvery === 0 ? { label: finitePrecision(value) } : {}),
    })
  }
  return ticks
}
