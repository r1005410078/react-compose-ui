 ...
 * 都不裁，实测五千多个节点一个没少。四分之一屏时工作档留下 2077 个节点、平移稳态 14ms，其中
 * Blink 的合成层划分 8ms 只与节点数有关；收到八分之一屏面积再降三成。带子随之变短，靠场景层
 * 的自适应批（长队列几帧内摊完）保证它仍在进入可视区之前建好。
 *
 * @internal
 */
const WINDOW_EXPANSION = 0.125
