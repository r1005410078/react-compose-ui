...
describe('StageInteractionController ECS systems', () => {
  ...
  it.each([
    ...
    // 旋转的入口改为指示器圆环；`rotate` 工具与 `rotate` 命中类型一并删除。
    ['rotate', entity('a', { resize: 'free', rotatable: false }), { kind: 'gizmo-handle', handle: 'rotate' }, 'select' as const],
  ] as const)(
...
describe('StageInteractionController 画布内文字编辑会话', () => {
  ...
  it('OpenSpec: 无 DOM 文字编辑会话 / 编辑期间变换手柄不产生空间手势', () => {
    for (const hit of [
      ...
      { kind: 'gizmo-handle', handle: 'rotate' },
    ] as const) {
      const { controller, effects } = textSetup({
        selectedIds: ['a'],
        textEditing: { entityId: 'a' },
