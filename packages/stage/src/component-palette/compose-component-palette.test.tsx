...
import { ComposeComponentPalette, ComposeStage } from '../index'
import type { ComposeStageFramePreset } from '../frame-preset'

type Registry = ReturnType<typeof createComposeComponentRegistry>
...
function Workspace({
  ...
  prefix: string
  framePresets?: readonly ComposeStageFramePreset[]
}) {
...
const framePreset: ComposeStageFramePreset = {
  id: 'desktop',
  label: 'Frame',
  name: 'Desktop Frame',
...
describe('ComposeComponentPalette', () => {
  ...
  // OpenSpec: stage / ComposeComponentPalette 拖入 / 拖到空白 Canvas
  ...
  it('OpenSpec: stage / ComposeComponentPalette 拖入 / 拖到嵌套 Frame', () => {
    const base = document()
  ...
  it('OpenSpec: stage / ComposeComponentPalette 拖入 / 隔离多个 Stage 实例', () => {
    const definitions = registry()
  ...
  it('OpenSpec: stage / ComposeComponentPalette 拖入 / 键盘新增路径', () => {
    const runtime = createTransactionRuntime({ document: document() })
    const events: string[] = []
    runtime.subscribeEvents((event) => events.push(event.type))
    const definitions = registry()
  ...
  it('OpenSpec: stage-engine / 统一外部拖入 / factory 异常不泄漏到 Palette', () => {
    const runtime = createTransactionRuntime({ document: document() })
    const controller = createStageInteractionController()
    const brokenPreset: ComposeStageFramePreset = {
      ...framePreset,
      id: 'broken',
      label: 'Broken Frame',
      createDefaultStyle: () => {
