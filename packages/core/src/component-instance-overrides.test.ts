...
/**
 * 组件解析文档必须是单 Frame 根；复用 componentDocumentFixture 获得合法默认值，
 * 否则失败会来自夹具而不是被测行为。
 */
...
describe('实例覆盖分区模型', () => {
  ...
  it('解析后文档不合法时不返回半应用结果', () => {
    const resolved = resolveComposeInstanceOverrides({
      document: snapshotDocument(),
      // 删掉根 Frame 会让文档失去唯一根，解析必须整体失败而不是返回半应用结果。
      overrides: { operations: [{ id: 'op-1', kind: 'remove-entity', entityId: ROOT_FRAME_ID }] },
    })
    expect(resolved.ok).toBe(false)
  })
})
