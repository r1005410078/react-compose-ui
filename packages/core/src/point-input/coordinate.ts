 ...
 *
 * @remarks
 * 刻意不用 `ComposePosition`：后者继承 `JsonObject`（带索引签名），普通的 `{ x, y }` 对象
 * 无法赋值给它。管线处理的是**尚未落进文档**的候选点，用结构类型才不会逼调用方到处重建对象。
 *
 * @public
 */
export interface ComposeInputPoint {
  readonly x: number
  readonly y: number
}

/** 坐标解析结果；失败时不给出任何近似点。 @public */
export type ParseComposeCoordinateResult =
  | { readonly ok: true; readonly point: ComposeInputPoint }
  | { readonly ok: false; readonly reason: ComposeCoordinateFailure }

/** 坐标解析失败的原因。 @public */
export type ComposeCoordinateFailure =
 ...
 *
 * 两块画布共用本实现：语法是用户的肌肉记忆，各写一份必然在某个写法上漂移。
 *
 * @param text - 用户键入的原始文本。
 * @param reference - 上一个点；相对与极坐标据此求解，缺省时这两种写法被拒绝。
 * @public
 */
export function parseComposeCoordinate(
  text: string,
