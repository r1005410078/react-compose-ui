...
独立解绑每个目标。默认 38px 操作列容纳两个 16px 图标：binding 优先，其后依次为存在性/新增、重置、删除
...
- 两条可聚焦分隔线分别调整属性名列和操作列；默认宽度为 120px/38px。在约 365px 的内容区，编辑列
  保留约 207px，优先容纳内容与并列输入；方向键移动 8px，
  Shift 加方向键移动 24px。操作列限制在 32–96px，默认两槽、最多三槽；空间不足时使用溢出菜单，行右键
...
- 嵌套结构只缩进标题、标签和引导线，编辑列边界保持全局对齐；深层缩进会自动封顶，避免属性名
  被层级空白挤压。
- 分组和字段的重置只在 `defaultValue` 能通过完整 Schema 时提供。宿主必须显式传入
  `defaultValue`，重置动作与“已修改”筛选才会出现；不传、或把当前 `value` 当作
  `defaultValue` 传入，都会让二者永久不可用。基线应来自宿主自己的默认值定义并保持稳定；
  确实没有实例无关默认值的字段（例如位置、尺寸），可让基线沿用当前值，使该字段不生成重置
  动作，同时不影响同一 Schema 中其他字段的重置。
