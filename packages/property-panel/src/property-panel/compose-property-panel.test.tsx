  ...
  it('OpenSpec: property-panel / 属性面板共享 UI 环境 / 使用英文浅色属性面板', () => {
    ...
    render(
      <ComposeUIProvider locale="en-US" theme="light">
        <ComposePropertyPanel schema={schema} value={{ items: [] }} />
      </ComposeUIProvider>,
    )
...
describe('OpenSpec: property-panel / Schema 类型自动映射 / 显示基础属性类型', () => {
  it('为基础类型显示匹配控件与约束', () => {
    render(<ComposePropertyPanel schema={basicSchema} value={basicValue} />)
...
describe('OpenSpec: property-panel / 有效受控变更 / 保留无效输入草稿', () => {
  it('数字在 blur 时校验，无效草稿保留到修正后提交', () => {
    ...
    const onValueChange = vi.fn()
    const { rerender } = render(
      <ComposePropertyPanel schema={schema} value={{ opacity: 1 }} onValueChange={onValueChange} />,
    )
    ...
    rerender(<ComposePropertyPanel schema={schema} value={{ opacity: 0.5 }} onValueChange={onValueChange} />)
...
function ChartRenderer({ value, commit }: import('../index').ComposePropertyPanelRendererProps) {
...
describe('OpenSpec: property-panel / 自定义类型 Renderer Registry / 使用全宽自定义 renderer', () => {
  ...
  it('全宽字段继续响应搜索、只读和 optional 存在性', () => {
    ...
    function ReadOnlyChartRenderer(props: import('../index').ComposePropertyPanelRendererProps) {
...
describe('OpenSpec: property-panel / 自定义类型 Renderer Registry / 缺少类型 renderer', () => {
  it('registry 保持实例隔离且不安全转换 custom 值', () => {
    ...
    render(<ComposePropertyPanel schema={chartSchema} value={{ chart: { series: [1] } }} />)
...
describe('OpenSpec: property-panel / 搜索筛选与默认值重置 / 搜索嵌套属性', () => {
  it('搜索字段说明并保留祖先，清空后恢复内容', () => {
    render(<ComposePropertyPanel defaultValue={panelDefaults} schema={panelSchema} value={panelValue} />)
...
describe('OpenSpec: property-panel / 搜索筛选与默认值重置 / 筛选修改或错误', () => {
  ...
  it('仅显示当前完整 Schema issue 所在字段及祖先', () => {
    ...
    render(<ComposePropertyPanel schema={schema} value={{ transform: { x: 20 }, name: 'Node' }} />)
...
describe('OpenSpec: property-panel / Metadata 驱动的展示属性 / 应用展示 metadata', () => {
  it('按 section 分组，并应用 hidden、readOnly 和初始折叠状态', () => {
    ...
    render(<ComposePropertyPanel schema={schema} value={{
      accent: '#3b82f6',
      locked: 'stable',
      hidden: 'secret',
...
describe('OpenSpec: property-panel / 嵌套与集合属性编辑 / 修改数组和元组', () => {
  ...
  it('tupleWithRest 保留固定项并允许新增和删除 rest 项', () => {
    ...
    function TupleRestHarness() {
      ...
      return <ComposePropertyPanel schema={schema} value={value} onValueChange={setValue} />
    }
...
describe('OpenSpec: property-panel / 受控属性变量绑定 / 绑定类型兼容变量', () => {
  it('OpenSpec: property-panel / 受控属性变量绑定 / 未声明字段不启用变量绑定', () => {
    expect(propertyPanelModule).toHaveProperty('resolveComposePropertyBindings')
    ...
    const result = resolveComposePropertyBindings({
      schema,
      value: { title: 'Literal' },
    ...
    const pointRenderer: ComposePropertyPanelRenderer = {
      id: 'point',
    ...
    const customResult = resolveComposePropertyBindings({
      schema: customSchema,
      value: { point: { x: 1 } },
  ...
  it('筛选兼容变量并在绑定、只读预览和解绑之间保持字面值', () => {
    ...
    function BindingHarness() {
      const [bindings, setBindings] = useState<readonly ComposePropertyPanelBinding[]>([])
      return (
        <ComposePropertyPanel
          binding={{
            value: bindings,
