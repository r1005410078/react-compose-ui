...
import { createComposeIndexedDbOperationLogStore } from './stores'
import type {
...
/** 操作日志 Provider 的配置。 */
export interface ComposeOperationLogProviderProps {
  /** 当前页面、文档或工作区的稳定隔离 ID。 */
  scopeId: string
  /** 由宿主提供的可选操作者。 */
...
const OperationLogContext = createContext<ComposeOperationLogController | null>(null)
 ...
 */
export function ComposeOperationLogProvider({
  scopeId,
  actor,
  maxEntries,
  maxSnapshotBytes,
  ...
  onStorageError,
  children,
}: ComposeOperationLogProviderProps) {
  const indexedDbStore = useMemo(() => createComposeIndexedDbOperationLogStore(), [])
  ...
  const controller = useMemo<ComposeOperationLogController>(() => ({
    status: state.status,
...
/**
 * 读取最近的 ComposeOperationLogProvider 控制器。
 *
 * @returns 当前 scope 的响应式日志状态和命令。
 * @throws 当调用位置不在 ComposeOperationLogProvider 子树中时抛出。
 */
// eslint-disable-next-line react-refresh/only-export-components -- Provider 与配套 Hook 必须从同一 React 模块共享私有 Context。
export function useComposeOperationLog(): ComposeOperationLogController {
  const controller = useContext(OperationLogContext)
  if (!controller) {
    throw new Error('useComposeOperationLog must be used inside ComposeOperationLogProvider')
  }
  return controller
}
