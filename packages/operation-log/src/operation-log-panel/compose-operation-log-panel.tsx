...
/** 操作日志查看面板属性。 */
export interface ComposeOperationLogPanelProps extends HTMLAttributes<HTMLDivElement> {
  /** 没有匹配记录时显示的文字。 */
  emptyMessage?: string
}
...
function getOperationLogMessages(
  ...
  const categoryFallback: Record<ComposeOperationLogCategory, string> = zh
  ...
  return {
    ...
    categories: Object.fromEntries(
      (Object.keys(categoryFallback) as ComposeOperationLogCategory[]).map((category) => [
        category,
        format(`category.${category}`, categoryFallback[category]),
      ]),
    ) as Record<ComposeOperationLogCategory, string>,
  }
}
...
function Snapshot({
  ...
  snapshot?: ComposeOperationLogSnapshot
}) {
...
function Detail({
  ...
  entry: ComposeOperationLogEntry
  locale: 'zh-CN' | 'en-US'
  messages: OperationLogMessages
  onClose: () => void
}) {
 ...
 */
export function ComposeOperationLogPanel({
  className,
  emptyMessage,
  'aria-label': ariaLabel,
  style,
  ...props
}: ComposeOperationLogPanelProps) {
    ...
    >
      ...
      <div className="operation-log__toolbar">
        ...
        <select
          ...
          value={category}
          onChange={(event) => setCategory(event.currentTarget.value as ComposeOperationLogCategory | 'all')}
        >
          <option value="all">{messages.allCategories}</option>
          {(Object.keys(messages.categories) as ComposeOperationLogCategory[]).map((value) => (
