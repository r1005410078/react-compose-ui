...
import { ComposeSceneTree, useComposeSceneTreeCommands } from '../index'
...
function renderTree(
  overrides: Partial<React.ComponentProps<typeof ComposeSceneTree>> = {},
  ...
  render(
    <ComposeUIProvider locale={locale}>
      <ComposeSceneTree
        nodes={nodes}
        selectedIds={[]}
        expandedIds={['page']}
...
describe('ComposeSceneTree', () => {
  ...
  it('OpenSpec: ui-context / 共享 UI Context 包 / ComposeSceneTree 消费 Context', () => {
    ...
    rerender(
      <ComposeUIProvider locale="en-US" theme="light">
        <ComposeSceneTree expandedIds={['page']} nodes={nodes} selectedIds={[]} />
      </ComposeUIProvider>,
    )
    expect(screen.getByRole('treegrid', { name: 'Scene tree' })).toBeInTheDocument()
  })
  ...
  it('deletes the focused unselected row instead of the stale selection', async () => {
    const unlockedNodes: readonly ComposeSceneTreeNode[] = [{
      ...nodes[0],
  ...
  it('OpenSpec: scene-tree / 场景树命令菜单与快捷键 / 外部与菜单共享命令状态', async () => {
    function Harness() {
      ...
      const controller = useComposeSceneTreeCommands({ nodes, selectedIds })
      return (
        <>
  ...
  it('OpenSpec: scene-tree / 场景节点操作意图 / 拖拽期间展开和滚动', async () => {
    const hoverNodes: readonly ComposeSceneTreeNode[] = [
