...
describe('ComposeCommandPanel', () => {
  it('OpenSpec: command-panel / 独立结构化命令调试台 / 显示三种命令结果', () => {
    const controller = runtime()
    render(<ComposeCommandPanel runtime={controller} aria-label="命令调试台" />)
  ...
  it('OpenSpec: command-panel / 独立结构化命令调试台 / 限制会话事件数量', () => {
    const controller = runtime()
    render(<ComposeCommandPanel eventLimit={2} runtime={controller} />)
  ...
  it('OpenSpec: command-panel / 独立结构化命令调试台 / 更换运行时', () => {
    ...
    const view = render(<ComposeCommandPanel runtime={first} />)
    act(() => first.dispatch(renameCommand('First')))
    expect(screen.getByText('rename-First')).toBeInTheDocument()

    view.rerender(<ComposeCommandPanel runtime={second} />)
    act(() => {
  ...
  it('OpenSpec: command-panel / 命令预设表单 / 派发有效预设', () => {
    ...
    render(<ComposeCommandPanel presets={[preset]} runtime={controller} />)
  ...
  it('OpenSpec: command-panel / 命令面板内建本地化 / 使用英文命令面板', () => {
    ...
    render(
      <ComposeUIProvider locale="en-US">
        <ComposeCommandPanel presets={[preset]} runtime={controller} />
      </ComposeUIProvider>,
    )
  ...
  it('OpenSpec: command-panel / 命令面板内建本地化 / 独立使用默认语言', () => {
    render(<ComposeCommandPanel runtime={runtime()} />)
    expect(screen.getByRole('region', { name: '命令调试台' })).toBeInTheDocument()
    expect(screen.getByText('暂无命令事件')).toBeInTheDocument()
  })

  it('OpenSpec: ui-context / 共享 UI Context 包 / 独立组件保持兼容 - ComposeCommandPanel 消费 Context', () => {
    const runtime = createTransactionRuntime({ document: fixture() })
    render(
      <ComposeUIProvider locale="en-US" theme="light">
        <ComposeCommandPanel presets={[]} runtime={runtime} />
      </ComposeUIProvider>,
    )
